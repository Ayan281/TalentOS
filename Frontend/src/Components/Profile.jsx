  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await api.post(
        "/api/upload/uploadResume", // ✅ no more hardcoded localhost
        formData,
      );
      ...
      if (error.response?.status === 403) {
        toast.error("Admins are not allowed to upload resumes", { id: toastId });
      } else {
        toast.error(
        ...
        <main className="flex-1 w-full max-w-[1400px] mx-auto px-6 md:px-12 pt-24 pb-20 flex flex-col lg:flex-row gap-16 items-center">
          <div className="w-full lg:w-5/12 pointer-events-none">
            <div className="inline-flex items-center gap-2 mb-6 pointer-events-auto bg-white/5 border border-white/10 px-4 py-2 rounded-full">
              ...
              <span className="text-xs font-medium text-white/80">Step 1: Setup Profile</span>
            </div>
            ...
            <p className="text-base md:text-lg text-white/60 max-w-md font-light mt-6 leading-relaxed pointer-events-auto">
              Upload your resume and details to let CampusOS match you with the best career opportunities and personalized roadmaps.
            </p>
          </div>

          <div className="w-full lg:w-7/12 pointer-events-auto relative">
            <form onSubmit={handleSubmit} className="backdrop-blur-2xl bg-[#ffffff08] border border-white/10 p-8 md:p-10 shadow-2xl relative overflow-hidden rounded-3xl">
              <div className="flex justify-between items-end border-b border-white/10 pb-6 mb-8">
                ...
                <span className="text-sm font-medium text-[#9DFF13]">Required</span>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="group relative">
                  <label className="text-sm font-medium text-white/70 mb-2 block group-focus-within:text-[#9DFF13] transition-colors">Full Name</label>
                ...
                <div className="group relative">
                  <label className="text-sm font-medium text-white/70 mb-2 block group-focus-within:text-[#9DFF13] transition-colors">Desired Job Role</label>
                  <input type="text" name="jobRole" value={jobRole} onChange={(e) => setJobRole(e.target.value)} required
                    className="w-full bg-[#0A0A0A]/50 border border-white/10 px-4 py-3.5 focus:outline-none focus:border-[#9DFF13] focus:ring-1 focus:ring-[#9DFF13] transition-all text-base text-white rounded-xl placeholder-white/20"
                    placeholder="e.g. Frontend Engineer" />
                </div>

                <div className="group relative md:col-span-2">
                  <label className="text-sm font-medium text-white/70 mb-2 block group-focus-within:text-[#9DFF13] transition-colors">Experience Level</label>
                  <div className="relative">
                    <select name="experience" value={experience} onChange={(e) => setExperience(e.target.value)} required
                      ...
                      <option value="" disabled className="bg-[#0A0A0A] text-white/50">Select your level...</option>
                      <option value="entry" className="bg-[#0A0A0A]">Entry Level (0-1 years)</option>
                      <option value="junior" className="bg-[#0A0A0A]">Junior (1-3 years)</option>
                      <option value="mid" className="bg-[#0A0A0A]">Mid-Level (3-5 years)</option>
                ...
                <div className="md:col-span-2 mt-2">
                  <label className="text-sm font-medium text-white/70 mb-2 block">Resume Upload</label>
                  <div
                    ...
                    onDrop={(e) => { e.preventDefault(); setIsDragging(false); setResume(e.dataTransfer.files[0]); }}
                  >
                    ...
                    {resume ? (
                      <div className="flex items-center gap-3 text-[#9DFF13]">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="bg-[#9DFF13]/20 p-1 rounded-full">
                          <path d="M22 11.08V12a10 10 0 1 1-5.93-9.14" />
                          <polyline points="22 4 12 14.01 9 11.01" />
                        </svg>
                        <span className="font-medium text-base text-white">{resume.name}</span>
                      </div>
                    ) : (
                      <>
                        ...
                        <p className="text-base font-medium text-white/90">Click or drag your file here</p>
                        <p className="text-xs text-white/40 mt-1">Supports PDF, DOC, DOCX (Max 5MB)</p>
                      </>
                    )}
                  </div>
                </div>
