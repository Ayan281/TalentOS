...
const SignUp = () => {
  ...
  function submitHandler(e) {
    ...
    const toastId = toast.loading("Creating account..."); // ✅ moved outside .then/.catch

    api
      .post("/api/auth/register", { // ✅ no more hardcoded localhost
        email,
        username,
        password,
        // ✅ role removed — backend hardcodes it to "user" now
      })
      .then((res) => {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white font-sans antialiased relative overflow-hidden selection:bg-[#9DFF13]/30">
      ...
      <div className="absolute top-[-10%] left-[-10%] w-[40%] h-[40%] bg-[#9DFF13] rounded-full filter blur-[150px] opacity-10 pointer-events-none"></div>
      <div className="absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] bg-[#ffffff] rounded-full filter blur-[150px] opacity-5 pointer-events-none"></div>
        ...
        <div className="w-full max-w-md bg-white/[0.02] backdrop-blur-3xl border border-white/[0.08] p-8 md:p-10 shadow-[0_8px_32px_rgba(0,0,0,0.5)] relative rounded-[2rem] group">
          ...
          <div className="mb-8 text-center relative z-20">
            ...
            <p className="text-white/50 text-sm font-medium leading-relaxed">
              Join CampusOS to unlock curated career opportunities and build your professional identity.
            </p>
          </div>
          ...
          <div className="flex items-center gap-4 my-8 relative z-20">
            ...
            <span className="text-xs text-white/40 font-medium px-2">or register with email</span>
            <div className="flex-1 h-[1px] bg-white/10" />
          </div>
